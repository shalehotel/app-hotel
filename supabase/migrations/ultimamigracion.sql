...
RETURNS TABLE (id uuid, serie text, numero integer, numero_completo text)
...
BEGIN
    ...
    FROM public.series_comprobante sc
    WHERE sc.serie = p_serie AND sc.tipo_comprobante = v_tipo_enum FOR UPDATE;
    IF v_correlativo IS NULL THEN
        RAISE EXCEPTION 'Serie % del tipo % no encontrada', p_serie, p_tipo_comprobante;
    END IF;
    UPDATE public.series_comprobante sc
    SET correlativo_actual = v_correlativo, updated_at = now()
    WHERE sc.serie = p_serie AND sc.tipo_comprobante = v_tipo_enum;
    ...
    RETURN QUERY SELECT
        v_comprobante_id AS id,
        p_serie AS serie,
        v_correlativo AS numero,
        v_numero_completo AS numero_completo;
END;
$$;
