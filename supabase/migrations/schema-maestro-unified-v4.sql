-- =============================================
-- SCHEMA COMPLETO MAESTRO - VERSIÓN UNIFICADA v4.0
...
--              NO requiere migraciones adicionales (excepto RLS).
-- =============================================
-- INTEGRA:
...
--   - 20260214_audit_minor_fixes.sql (idx_comprobante_detalles + REPLICA IDENTITY caja_turnos) ← ABSORBIDA
--   - 20260214_check_metodo_pago.sql (CHECK constraints metodo_pago en pagos y movimientos) ← ABSORBIDA
-- =============================================
-- MIGRACIONES SEPARADAS (NO INTEGRADAS):
--   - 20260214_rls_policies_by_role.sql → Modelo de seguridad RLS por rol (ADMIN/RECEPCION/HOUSEKEEPING)
--     Razón: Cambio de arquitectura de seguridad; debe ser reversible independientemente.
-- =============================================
...
CREATE TABLE public.caja_movimientos (
    ...
    CONSTRAINT check_mov_metodo_pago_valido CHECK (metodo_pago IS NULL OR metodo_pago IN (
...
CREATE TABLE public.pagos (
    ...
    CONSTRAINT check_metodo_pago_valido CHECK (metodo_pago IN (
        'EFECTIVO', 'TARJETA', 'TRANSFERENCIA', 'YAPE', 'PLIN',
        'DEVOLUCION_EFECTIVO', 'DEVOLUCION_TARJETA', 'DEVOLUCION_TRANSFERENCIA',
        'DEVOLUCION_YAPE', 'DEVOLUCION_PLIN', 'DEVOLUCION_PENDIENTE'
    ))
...
-- Índice FK en comprobante_detalles (evita full table scan en JOINs por comprobante)
CREATE INDEX IF NOT EXISTS idx_comprobante_detalles_comprobante ON public.comprobante_detalles(comprobante_id);
...
-- RLS: DESACTIVADO en este schema base (estado inicial).
-- La migración 20260214_rls_policies_by_role.sql lo ACTIVA con políticas granulares
-- por rol (ADMIN / RECEPCION / HOUSEKEEPING). Aplicar ESA migración DESPUÉS de este schema.
...
-- REPLICA IDENTITY FULL en caja_turnos: necesario para que los UPDATE events
-- de Supabase Realtime envíen TODOS los campos al cliente (no solo PKs).
ALTER TABLE caja_turnos REPLICA IDENTITY FULL;
...
-- 11. FIN
-- =============================================
DO $$ BEGIN RAISE NOTICE '✅ SYSTEM READY: SCHEMA UNIFICADO v4.0 CREADO CON ÉXITO'; END $$;
-- Para activar seguridad por roles, ejecutar a continuación:
-- \i 20260214_rls_policies_by_role.sql
