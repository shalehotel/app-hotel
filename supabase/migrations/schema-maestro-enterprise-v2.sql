-- =============================================
-- SCHEMA COMPLETO MAESTRO - VERSIÓN CONSOLIDADA ENTERPRISE v2.0
-- TIPO: MIGRACIÓN BASE (GOLD MASTER)
-- FECHA: 2026-01-25
-- DESCRIPCIÓN: Integra esquema original COMPLETO + mejoras enterprise + refactor devoluciones.
-- =============================================
...
CREATE TABLE public.pagos (
    ...
    -- Enterprise: Idempotencia y validación de cero
...
-- Índices adicionales del esquema original para reportes
...
-- RPC: Insertar Comprobante Atómico (RESTAURADA)
-- Esencial para facturación y notas de crédito. Garantiza correlativos sin huecos.
...
-- RPC: Registrar Cobro Completo (Enterprise)
...
BEGIN
    ...
    -- Lógica interna de cobro (similar a insertar_comprobante_atomico pero unificada con pagos)
    -- Se mantiene por compatibilidad con módulo de Checkout rápido
    ...
    VALUES (p_turno_caja_id, p_usuario_id, 'INGRESO', 'COBRO_SERVICIO', p_moneda::moneda_enum, p_monto_pago, 'Cobro Reserva ' || COALESCE(v_codigo_reserva, p_reserva_id::text) || ' - ' || p_metodo_pago, v_numero_completo, p_metodo_pago);
...
-- RPC: Procesar Devolución Atómica (IDEMPOTENTE)
...
BEGIN
    ...
    IF p_metodo_devolucion = 'EFECTIVO' AND p_turno_id IS NOT NULL THEN
        INSERT INTO public.caja_movimientos (caja_turno_id, usuario_id, tipo, categoria, moneda, monto, motivo, comprobante_referencia, metodo_pago)
        VALUES (p_turno_id, p_usuario_id, 'EGRESO', 'DEVOLUCION', v_reserva.moneda_pactada, p_monto_devolucion, 'Devolución Acortamiento', v_reserva.codigo_reserva, 'EFECTIVO') RETURNING id INTO v_movimiento_id;
    END IF;
...
-- RPC: Marcar Devolución Procesada
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_pago record; v_turno_activo uuid;
BEGIN
    SELECT * INTO v_pago FROM public.pagos WHERE id = p_pago_id AND metodo_pago = 'DEVOLUCION_PENDIENTE' FOR UPDATE;
    IF v_pago IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Pago no encontrado'); END IF;
    UPDATE public.pagos SET metodo_pago = p_metodo_real, nota = nota || ' | Procesado: ' || p_metodo_real, fecha_pago = now() WHERE id = p_pago_id;
    IF p_metodo_real = 'EFECTIVO' THEN
        SELECT id INTO v_turno_activo FROM public.caja_turnos WHERE estado = 'ABIERTA' LIMIT 1;
        IF v_turno_activo IS NOT NULL THEN
            INSERT INTO public.caja_movimientos (caja_turno_id, usuario_id, tipo, categoria, moneda, monto, motivo, comprobante_referencia, metodo_pago)
            VALUES (v_turno_activo, auth.uid(), 'EGRESO', 'DEVOLUCION_PROCESADA', v_pago.moneda_pago, ABS(v_pago.monto), 'Devolución diferida', NULL, 'EFECTIVO');
        END IF;
    END IF;
...
-- Vista Devoluciones Pendientes (Refactor)
...
-- =============================================
-- 8. FUNCIONES OPERATIVAS ADICIONALES (RECUPERADAS)
-- =============================================
...
-- Insertar Comprobante Atómico (Helper)
CREATE OR REPLACE FUNCTION insertar_comprobante_atomico(
  p_serie TEXT, p_tipo_comprobante TEXT, p_turno_caja_id UUID, p_reserva_id UUID, p_receptor_tipo_doc TEXT, p_receptor_nro_doc TEXT, p_receptor_razon_social TEXT, p_receptor_direccion TEXT, p_moneda TEXT, p_tipo_cambio DECIMAL, p_op_gravadas DECIMAL, p_op_exoneradas DECIMAL, p_monto_igv DECIMAL, p_total_venta DECIMAL, p_nota_credito_ref_id UUID DEFAULT NULL
) RETURNS TABLE(id UUID, serie TEXT, numero INTEGER, numero_completo TEXT) LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_correlativo INTEGER; v_serie_id UUID; v_nuevo_id UUID;
BEGIN
  SELECT sc.id, sc.correlativo_actual + 1 INTO v_serie_id, v_correlativo FROM series_comprobante sc WHERE sc.serie = p_serie AND sc.tipo_comprobante = p_tipo_comprobante::tipo_comprobante_enum FOR UPDATE;
  IF v_serie_id IS NULL THEN RAISE EXCEPTION 'Serie no encontrada'; END IF;
  INSERT INTO comprobantes (turno_caja_id, reserva_id, tipo_comprobante, serie, numero, receptor_tipo_doc, receptor_nro_doc, receptor_razon_social, receptor_direccion, moneda, tipo_cambio, op_gravadas, op_exoneradas, op_inafectas, monto_igv, monto_icbper, total_venta, nota_credito_ref_id, estado_sunat, fecha_emision)
  VALUES (p_turno_caja_id, p_reserva_id, p_tipo_comprobante::tipo_comprobante_enum, p_serie, v_correlativo, p_receptor_tipo_doc, p_receptor_nro_doc, p_receptor_razon_social, p_receptor_direccion, p_moneda::moneda_enum, p_tipo_cambio, p_op_gravadas, p_op_exoneradas, 0, p_monto_igv, 0, p_total_venta, p_nota_credito_ref_id, 'PENDIENTE', NOW()) RETURNING comprobantes.id INTO v_nuevo_id;
  UPDATE series_comprobante SET correlativo_actual = v_correlativo WHERE series_comprobante.id = v_serie_id;
  RETURN QUERY SELECT v_nuevo_id, p_serie, v_correlativo, (p_serie || '-' || LPAD(v_correlativo::TEXT, 8, '0'));
END; $$;

-- =============================================
-- 9. PERMISOS (CRÍTICO - RECUPERADO)
-- =============================================
...
DO $$ BEGIN RAISE NOTICE '✅ SYSTEM READY: SCHEMA ENTERPRISE V2 RECONSTRUIDO CON ÉXITO'; END $$;
